...
<p>Если нужно, чтобы element был равен "some value", то в эталонном файле должно быть указано следующее
```
"element": {
  "matchType": "equals",
  "value":"some value"
...
"Мягкое" равенство: сравнение будет производиться только по значениям, указанным в объекте **value** 
```
"element": {
  "matchType": "equals",
  "value": {
...
* DIST_DAY
  К дате из массива value прибавляется число из этого же массива -- полученная дата должна совпадать с actual значением.
...
```
* IN_LIST
  данный оператор используется для проверки того, что элемент может принимать одно из допустимых значений. Допустим, что
  element должен принимать значения только 15, 20, 25
```
"element": {
  "matchType": "in_list",
  "value": [15, 20, 25]
...
* OF_TYPE
  можно проверить, что значение элемента имеет определенный тип, например
```
"element": {
  "matchType": "of_type",
  "value": "ARRAY"
...
* EQUALS_AS_SET
Этот оператор нужно использовать, когда необходимо, чтобы в массиве содержались только определенные элементы и при этом их порядок не важен
```
"element": {
...
* EQUALS_AS_SET_
"Мягкий" оператор сравнения массивов используется, если массив содержит объекты и в каждом объекте допустимы дополнительные поля, кроме указанных
...
* CONTAINS_ONLY_ 
Проверяет, что все элементы массива равны указанному, но только в объеме полей, присутствующих в элементе
...
* CONTAINS
Если значение элемента - массив и нужно проверить, что в нем содержится одно или несколько значений, используй contains
...
* CONTAINS_
Для массива примитивных значений работает как простой CONTAINS.
...
* NOT_CONTAINS_
  Для массива примитивных значений работает как простой NOT_CONTAINS.
  Если в массиве присутствуют объекты, то сравнение будет происходить только по полям эталонного объекта.
...
* NOT_EXISTS
проверяет, что объект не существует. Value указывать не нужно.
...
* NOT_EXISTS_
проверяет, что массив не содержит определённый элемент или вообще не существует
...
* MATCH_LIST - позволяет осуществлять несколько проверок для одного объекта.
